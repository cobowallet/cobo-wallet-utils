...
export const splitData = (data, capacity, compress=true, rate) => {
  ...
  const maybeCompressedData = compress? zlib.gzipSync(data).toString('base64'): data;
  ...
  for (let i = 0; i < partCount; i++) {
    parts.push(maybeCompressedData.substring(partLength * i, Math.min(partLength * i + partLength, maybeCompressedData.length)))
  }
...
export const extractData = (data) => {
  ...
  const maybeCompressedData = received.reduce((acc, current)=> acc + current.value, '');
  console.log(maybeCompressedData);
  if (verifyCheckSum(maybeCompressedData, checkSum)) {
    console.log(zlib.gunzipSync(Buffer.from(maybeCompressedData,'base64')).toString());
    return compress? zlib.gunzipSync(Buffer.from(maybeCompressedData,'base64')).toString() : maybeCompressedData;
  }
  else{
